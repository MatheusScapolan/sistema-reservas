...
Para Cleber, que "ama usar as salas de reunião" e busca eficiência, a capacidade de modificar uma reserva de forma descomplicada e transparente é um diferencial importante, tornando o sistema mais útil, adaptado às suas necessidades reais como estudante do INTELI e reforçando a percepção de um sistema estruturado e confiável.ELI.

### 3.3.7 Diagrama do Banco de Dados (Entidade-Relacionamento)

**Imagem de Referência:** `image.png` (primeira ocorrência, com o diagrama)

**Descrição:** O diagrama entidade-relacionamento (DER) fornecido ilustra a estrutura fundamental do banco de dados que suporta o sistema de reserva de salas. Ele define as principais entidades (tabelas), seus atributos (colunas) e os relacionamentos entre elas. Este diagrama é crucial para o desenvolvimento do backend e para garantir a integridade e consistência dos dados.

**Entidades Principais e Seus Atributos Chave:**

*   **`users` (Usuários):** Armazena informações sobre os usuários do sistema.
    *   `id` (PK, bigint): Identificador único do usuário.
    *   `nome_completo` (varchar): Nome completo do usuário.
    *   `email_institucional` (varchar, unique): Email institucional, usado para login.
    *   `senha` (varchar): Senha do usuário (deve ser armazenada de forma segura, com hash).
    *   `created_at` (timestamp): Data e hora de criação do registro.
    *   `updated_at` (timestamp): Data e hora da última atualização do registro.
*   **`rooms` (Salas):** Armazena informações sobre as salas disponíveis para reserva.
    *   `id` (PK, bigint): Identificador único da sala.
    *   `nome_sala` (varchar): Nome ou identificação da sala (ex: "Sala A101", "Auditório").
    *   `capacidade` (int): Número máximo de pessoas que a sala comporta.
    *   `has_tv` (bool): Indica se a sala possui TV.
    *   `has_whiteboard` (bool): Indica se a sala possui lousa.
    *   `descricao` (text): Descrição adicional ou observações sobre a sala.
    *   `created_at` (timestamp): Data e hora de criação do registro.
    *   `updated_at` (timestamp): Data e hora da última atualização do registro.
*   **`bookings` (Reservas):** Tabela central que armazena os detalhes das reservas feitas pelos usuários.
    *   `id` (PK, bigint): Identificador único da reserva.
    *   `user_id` (FK, bigint): Chave estrangeira referenciando `users.id`. Indica qual usuário fez a reserva.
    *   `room_id` (FK, bigint): Chave estrangeira referenciando `rooms.id`. Indica qual sala foi reservada.
    *   `data_reserva` (date): Data para a qual a sala foi reservada.
    *   `horario_inicio` (time): Horário de início da reserva.
    *   `horario_fim` (time): Horário de término da reserva.
    *   `motivo_reserva` (text, nullable): Motivo ou finalidade da reserva (opcional).
    *   `status_reserva` (varchar): Status atual da reserva (ex: "Confirmada", "Cancelada", "Realizada").
    *   `created_at` (timestamp): Data e hora de criação do registro.
    *   `updated_at` (timestamp): Data e hora da última atualização do registro.
*   **`booking_history` (Histórico de Reservas/Modificações):** Registra o histórico de alterações ou eventos importantes relacionados a uma reserva. Isso é útil para auditoria e rastreamento.
    *   `id` (PK, bigint): Identificador único do registro de histórico.
    *   `booking_id` (FK, bigint): Chave estrangeira referenciando `bookings.id`. Indica a qual reserva este histórico pertence.
    *   `usuario_modificador_id` (FK, bigint, nullable): Chave estrangeira referenciando `users.id`. Indica qual usuário realizou a modificação (pode ser o próprio usuário da reserva ou um administrador).
    *   `acao_realizada` (varchar): Descreve a ação que foi tomada (ex: "Criada", "Modificada", "Cancelada").
    *   `detalhes_alteracao` (text, nullable): Detalhes específicos sobre o que foi alterado.
    *   `timestamp_alteracao` (timestamp): Data e hora em que a alteração ocorreu.

**Relacionamentos:**

*   **`users` (1) -- (N) `bookings`:** Um usuário pode fazer várias reservas, mas cada reserva pertence a um único usuário.
*   **`rooms` (1) -- (N) `bookings`:** Uma sala pode ser reservada várias vezes (em horários diferentes), mas cada reserva específica é para uma única sala.
*   **`bookings` (1) -- (N) `booking_history`:** Uma reserva pode ter vários registros de histórico (ex: criação, múltiplas modificações, cancelamento), mas cada registro de histórico pertence a uma única reserva.
*   **`users` (1) -- (N) `booking_history` (via `usuario_modificador_id`):** Um usuário (modificador) pode estar associado a vários registros de histórico de modificação.

**Importância para o Projeto:**

Este DER é a espinha dorsal do sistema. Ele define como os dados são organizados, o que garante que as funcionalidades (como buscar salas por disponibilidade, verificar conflitos de horário, listar reservas de um usuário) possam ser implementadas de forma eficiente e confiável. A clareza dos relacionamentos e a escolha adequada dos tipos de dados são fundamentais para a performance e escalabilidade do sistema.

### 3.3.8 Persona (Cleber Alves)

**Imagem de Referência:** `image.png` (segunda ocorrência, com a persona)

**Descrição:** A persona "Cleber Alves" foi criada para representar um tipo de usuário chave do sistema de reserva de salas do INTELI. Entender as características, necessidades, dores e motivações de Cleber ajuda a guiar o design e o desenvolvimento do sistema, garantindo que ele seja centrado no usuário e atenda às expectativas reais.

**Detalhes da Persona:**

*   **Nome:** Cleber Alves
*   **Idade:** 20 anos
*   **Ocupação:** Estudante de Sistemas de Informação no INTELI.
*   **Características Chave:** "Ama usar as salas de reunião".
*   **Hábitos Digitais:**
    *   Usa Notion, Google Calendar e Discord para se organizar.
    *   Além da plataforma AdaLove.
    *   Prefere sistemas simples, objetivos e acessíveis por celular ou notebook.
*   **Motivações e Necessidades:**
    *   Agendar salas de forma rápida, centralizada e confiável.
    *   Ter uma visão clara da disponibilidade das salas.
    *   Evitar conflitos de horário.
*   **Dores (Problemas Atuais):**
    *   Já passou por conflitos de reserva.
    *   Falta de clareza sobre a disponibilidade de salas.
    *   Sente o processo atual como desorganizado e informal.
*   **Solução (O que o sistema deve oferecer para Cleber):**
    *   O sistema permite reservas rápidas e estruturadas.
    *   Visão em tempo real da disponibilidade.
    *   Evita sobreposições.
    *   Otimiza sua rotina acadêmica.

**Implicações para o Design dos Wireframes e do Sistema:**

As características de Cleber Alves reforçam a necessidade de um sistema com as seguintes qualidades, que foram consideradas no desenvolvimento dos wireframes:

*   **Simplicidade e Objetividade:** As telas de login, cadastro, busca e reserva são diretas, sem excesso de informações ou passos desnecessários.
*   **Acessibilidade:** Embora os wireframes sejam para desktop, a preferência de Cleber por acesso mobile sugere que o design final deve ser responsivo.
*   **Rapidez e Eficiência:** O fluxo para buscar e reservar uma sala (`Desktop 4.png`) é projetado para ser rápido. A visualização de "Minhas Reservas" (`Desktop 5.png`, `Desktop 6.png`) e a modificação (`Desktop 7.png`) também buscam agilidade.
*   **Clareza na Disponibilidade:** A tela de busca com filtros e resultados claros é essencial para atender a essa necessidade.
*   **Prevenção de Conflitos:** O sistema, através de seu backend e lógica de negócios (suportado pelo DER em `image.png`), deve garantir que não ocorram reservas conflitantes, aliviando uma das principais dores de Cleber.
*   **Centralização:** Um sistema único para todas as reservas de sala atende à necessidade de um processo centralizado e formal, contrastando com a percepção atual de desorganização.

A persona Cleber Alves serve como um lembrete constante de quem são os usuários finais e quais problemas o sistema "INTELI SALAS" se propõe a resolver, garantindo que as decisões de design e funcionalidade estejam alinhadas com suas necessidades e expectativas.
